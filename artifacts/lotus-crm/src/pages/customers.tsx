...
import { Search, Plus, Phone, MapPin, Tag, MoreHorizontal, User, FileText } from "lucide-react";
    ...
    <div className="flex-1 space-y-6 p-8 overflow-y-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage and view patient histories across all branches.</p>
        </div>
          ...
          <TableBody>
            {isLoading ? (
              ...
              customers?.map((customer) => (
                <TableRow key={customer.id} className="hover:bg-muted/50 cursor-pointer transition-colors group">
                  <TableCell>
                  ...
                  <TableCell>
                    <div className="flex gap-1 flex-wrap">
                      {customer.tags?.map((tag, idx) => (
                        <Badge key={idx} variant="secondary" className="font-normal text-[10px] px-1.5 h-5 bg-background">
                          {tag}
                        </Badge>
                      ))}
                    </div>
